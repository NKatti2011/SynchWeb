...
class Authentication
{
    ...
    // function __construct(App $app, $db) {
    //     $this->app = $app;
    //     $this->db = $db;


    //     $app->get('/authenticate/key', function (Request $request, Response $response) {
    //         return $this->send_response($response, 200, json_encode(array('key' => base64_encode(openssl_random_pseudo_bytes(64)))));
    //     });

    //     $app->get('/authenticate/check', function (Request $request, Response $response) use ($app) {
    ...
    //             if (sizeof($userc)) {
    //                 return $this->send_response($response, 200, $this->generate_jwt($user));
    //             }
    //         }
    //         return $this->send_response($response, 400, 'No previous session');
    //     });

    //     $app->get('/authenticate/logout', function (Request $request, Response $response) use ($app) {
    //     });

    //     $app->get('/authenticate', function (Request $request, Response $response) use ($app) {
    //         global $authentication_type;
    //         $user = $this->authenticateByType($authentication_type)->check();
    //         return $this->send_response($response, 200, $this->generate_jwt($user));
    ...
    //     // $this->app->post('/authenticate', array(&$this, 'authenticate'));
    //     // ->conditions(array(
    //     //     'login' => '[A-z0-9]+',
    //     //     'password' => '.*',
    //     // ));
    ...
    public function doLogin($req, $res, $args) {
        ...
        if ($this->authenticateByType($authentication_type)->authenticate($login, $password)) {
            $jwt = $this->generate_jwt($login);
            // Save session and continue process
            $_SESSION['user'] = $jwt;
            $this->_output(200, $jwt);
            return $res
    ...
    function send_response(Response $response, int $status_code, string $message) {
        $response->withStatus($status_code)
            ->withHeader('Content-Type', 'application/json')
            ->withAddedHeader('X-PHP-Response-Code', $status_code)
